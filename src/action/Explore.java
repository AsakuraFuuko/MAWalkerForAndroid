	...
	public static boolean run() throws Exception {
		...
		try {
			...
			if (ex.getMessage().startsWith("302")) {
				Process.AddUrgentTask(Info.EventType.innerMapJump);
				return false;
			}
		...
		try {
			...
			case 5:
				// floor or area clear
				if (Boolean.valueOf(xpath.evaluate("count(//next_floor)>0", doc,
			...
			case 12:
				...
				Process.info.ExploreResult = String.format("AP recover(%d)",
						Integer.parseInt(xpath.evaluate("//explore/recover",
								doc)));
				break;
			case 13:
				// BC
