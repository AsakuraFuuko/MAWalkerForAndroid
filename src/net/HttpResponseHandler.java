...
public final class HttpResponseHandler implements ResponseHandler<byte[]> {
	...
	/*
	 * private String preProcessResponse(HttpResponse hr){ if
	 * (hr.getAllHeaders().length > 0){ for(Header h:hr.getAllHeaders()){ if
	 * (h.getName().contains("Content-Spare-Item")){ return
	 * h.getElements()[0].getName(); } } } return ""; }
	 */
}
